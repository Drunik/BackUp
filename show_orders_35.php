...
//				$("#debug").html(html);  
	...
	$(document).ready(function() {
		...
		$("#settings_tabs li a").click(function() {
			...
			if (activeTab=='#idDispetchers'){
				show_dispetchers(<?php echo $_SESSION['company_id']?>);
			};			
			if (activeTab=='#idCars'){
				ShowCars(<?php echo $_SESSION['company_id']?>);
			};
...
function show_dispetchers(){
            $.ajax({  
	        url: "./my_source_codes/show_dispetchers.php?company_id=<?php echo $_SESSION['company_id']?>",  
                cache: false,  
                success: function(html){  
					$("#Dispetchers").html(html); 
                }  
            });  
			$(".tab_content").hide();
			var activeTab = "#Dispetchers";
			$(activeTab).fadeIn();
}
function add_dispetcher(){
            $.ajax({  
...
function del_dispetcher(id_){
            $.ajax({  
	           url: "./my_source_codes/del_dispetcher.php?id="+id_
			   , cache: false,  
                success: function(html){  
						$("#debugggg").html(html); 
                }  
...
function edit_ext_tarif(period_id_, div_table_tr_, id_, field_){
	var
	 ...
	 $car_class_id = id_,
	 $div_table_tr = 
	 $ratio_1 = $(div_table_tr_+" #ratio_1"+id_).val(),
	 $ratio_2 = $(div_table_tr_+" #ratio_2"+id_).val(),
	 $ratio_3 = $(div_table_tr_+" #ratio_3"+id_).val(),
	 $price_1 = $(div_table_tr_+" #price_1"+id_).val(),
	 $price_2 = $(div_table_tr_+" #price_2"+id_).val(),
	 $price_3 = $(div_table_tr_+" #price_3"+id_).val(),
	 $field = "#"+field_;
            $.ajax({  
